...
export function ShareButtons({ url, title }: { url: string; title: string }) {
  ...
  return (
    <div className="flex items-center gap-2">
      <span className="text-xs font-medium text-zinc-500 mr-1">Share:</span>
      <a
        href={`https://twitter.com/intent/tweet?text=${shareText}&url=${shareUrl}`}
        target="_blank"
        rel="noopener noreferrer"
        className="px-3 py-1.5 rounded-lg bg-zinc-100 dark:bg-zinc-800 text-xs font-medium hover:bg-zinc-200 dark:hover:bg-zinc-700 transition-colors"
      >
        Twitter
      </a>
      <a
...
export function ShareSidebar({ url, title }: { url: string; title: string }) {
  ...
  return (
    <aside className="hidden lg:flex flex-col items-center gap-3 pt-2 w-10 shrink-0">
      <span className="text-[10px] font-semibold uppercase tracking-wider text-zinc-400 [writing-mode:vertical-lr]">
        Share
      </span>
      <a
        href={`https://twitter.com/intent/tweet?text=${shareText}&url=${shareUrl}`}
